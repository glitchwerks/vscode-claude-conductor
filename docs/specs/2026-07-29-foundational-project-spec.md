...
Decomposing that into the specific failures a plain terminal produces. Items 1-4 are this project's own decomposition of the single cited claim above (`README.md:L13`, "painful in a plain terminal") into concrete failure modes — each is `unverified: inference derived from the cited product framing`, not independently sourced per item, since no separate citation exists for any one of them individually. Item 5 draws on its own distinct cited source and is treated separately below.
...
5. **Multi-root work fights Claude's own scoping guidance.** Anthropic's own guidance is that *"Claude works best when it's scoped to the part of the codebase that's actually relevant to the task"* ([How Claude Code works in large codebases: best practices and where to start](https://claude.com/blog/how-claude-code-works-in-large-codebases-best-practices-and-where-to-start), fetched 2026-07-30). That source supports "scoped initialization works best"; it does **not** itself say multi-root users are forced into a hop-or-oversize tradeoff — the following sentence is `unverified: this project's own inference extrapolating from the cited guidance, not an Anthropic claim`. A plain terminal gives a multi-root user no way to honor that scoping guidance across projects at once: they either hop between narrowly-scoped per-project sessions and lose continuity re-establishing context each time, or keep one large session with the context intact but running far from the source of the work it needs to touch.
...
|---|---|---|
| D-1 | **VS Code version floor disagrees.** README states the requirement as *"VS Code 1.85 or newer"*, but the manifest declares a higher floor. The manifest is what VS Code enforces, so the README understates the true floor. | `README.md:L115` ("VS Code 1.85 or newer"); `package.json:L9` (`"vscode": "^1.93.0"`); `package.json:L217` (`@types/vscode` pinned `^1.93.0`) |
| D-2 | **`launchDelayMs` is undocumented.** The setting exists but the README configuration table lists only the other five settings. | `package.json:L174-179` (setting declaration); `README.md:L71-77` (configuration table, `launchDelayMs` absent) |
| D-3 | **Duplicate output channel verified; fix pending merge.** Two independent `vscode.window.createOutputChannel("Claude Conductor")` call sites produced duplicate entries in the Output dropdown; resolved by PR #112 (https://github.com/glitchwerks/vscode-claude-conductor/pull/112), pending merge, by making `output.ts` the sole owner and routing `stateWatcher` logging through its exported `log` function. | `src/output.ts:L9-L13`; `src/stateWatcher.ts:L7,L92-L364`; issue #111 |
| D-4 | **Stale docstring on `uninstallHooks`.** Its comment reads *"Called on deactivate()"*, but `deactivate()` deliberately does not call it and says so. The only caller is the `removeHooks` command. The behaviour is correct; the docstring is wrong. | `src/hookInstaller.ts:L302-303` (stale docstring); `src/extension.ts:L232-236` (`deactivate()` explains hooks are intentionally left installed); `src/extension.ts:L185-188` (`removeHooks` is the only caller) |
