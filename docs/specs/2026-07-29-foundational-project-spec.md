...
| D-3 | **Duplicate output channel verified and fixed.** Two independent `vscode.window.createOutputChannel("Claude Conductor")` call sites produced duplicate entries in the Output dropdown; issue #111 fixed this by making `output.ts` the sole owner and routing `stateWatcher` logging through its exported `log` function. | `src/stateWatcher.ts:L7`; `src/output.ts:L11`; issue #111 |
