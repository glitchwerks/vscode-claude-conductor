...
- **v5 (2026-08-02)** — retroactive correction, not a design change. PR #77 (open, verified 2026-08-02) — its CodeRabbit review (triage finding #3, `.tmp/2026-08-01-pr77-triage.md`) flagged that the § Summary sentence above described two-level nesting (worktrees under a favorited project root, reusing `groupByProjectRoot`) that was never implemented. The shipped, reviewed code renders Favorites as a flat single-level list (`src/treeView.ts:L327-L347`), and `FavoritesStore.add()`/`.relocate()` reject worktree paths outright (`docs/plans/2026-04-28-75-favorites.md:L691`, `:L723`), so there is no worktree child to nest. Decision: keep the shipped flat implementation and correct the spec text to match, rather than change code to match the original (incorrect) spec. § Summary rewritten accordingly; no code or test changes.

## Summary

Add a third top-level tree view, **Favorites**, to the Claude Conductor sidebar. It sits between **Active Sessions** and **Recent Projects** and renders each favorited project root as a single flat row — no nested children. As shipped, `FavoritesProvider.getChildren` (`src/treeView.ts:L327-L347`) returns a flat list built from `FavoritesStore.list()`; there is no per-favorite child level. This diverges from the two-level `groupByProjectRoot` nesting used in Recent Projects, and intentionally so: `FavoritesStore.add()` and `.relocate()` both reject worktree paths with `"Favorite the project root, not a worktree."` (`docs/plans/2026-04-28-75-favorites.md:L691`, `:L723`), so a favorite is always a project root and there is never a worktree child to nest.
