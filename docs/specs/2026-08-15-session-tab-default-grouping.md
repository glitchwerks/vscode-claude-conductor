...
**Type:** feature-spec
**Status:** ACCEPTED — Rev 5; the § 2.5 probe session ran and **P-LABEL** (case (a), the sole hard gate) **PASSED**. Full results posted to [#127 comment `5305122522`](https://github.com/glitchwerks/vscode-claude-conductor/issues/127#issuecomment-5305122522), 2026-08-16 (fetched 2026-08-16). The § 2.8 document-state updates ran in the same commit.
...
| 5 | 2026-08-16 | **Accepted — probe session run, hard gate passed.** The owner ran the § 2.5.1 runbook per Rev 4. **P-LABEL case (a) PASSED**: an editor-born session tab's `Tab.label` settled to exactly `claude · <folder>` on both readings sampled (~13 s and ~23 s post-launch), matching `startsWith("claude · ")` undecorated — the spec is unblocked for implementation. All three informative checks also came back favourably and changed no design branch: **P6** — `Beside` always creates a new group rather than reusing an existing side group (OQ-5 stands as resolved); **P-PLACE** — `location: {viewColumn: N}` is honoured for an existing, non-active target group; **P-REVEAL** — `focusSession` still reveals an editor-born tab. Full results: [#127 comment `5305122522`](https://github.com/glitchwerks/vscode-claude-conductor/issues/127#issuecomment-5305122522). Status flips to ACCEPTED per § 2.5's stated rule; § 2.8's document-state updates (this spec's own Status line, the #110 plan's Status line, and the two `docs/README.md` rows) run in the same commit. |
...
**Resolved by § 2.5 P6, not left `unverified:`.** `Beside` **always creates** a new group; it does not reuse an existing side group. Setup: group 1 = source files, active; group 2 (`viewColumn=2`) = a pre-existing user-owned `powershell` terminal. After launching against `Beside`, `tabGroups.all.length` went from 2 to 3 — the session landed in a *new* `viewColumn=2` group and the existing `powershell` group was pushed to `viewColumn=3`, rather than the session joining it. [#127 comment `5305122522`](https://github.com/glitchwerks/vscode-claude-conductor/issues/127#issuecomment-5305122522), 2026-08-16 (fetched 2026-08-16).

**This hazard does not occur.** Had `Beside` reused, the empty state could have landed the first session in the user's group — user's code in group 1 (active), user's own terminal in group 2, session 1 lands in group 2 — after which group 2 would hold the majority of Conductor tabs and subsequent sessions would join it there. P6 shows `Beside` always creates instead, so the cold-start case FR3's caveat described never lands a session in a pre-existing user group; it is de-risked rather than merely stated, and OQ-5's "no fallback" answer stands unchanged either way (§ 2.5 P6 row).
...
3. **Fully cold, concurrent.** Both return `undefined`, both request `Beside`. P6 resolves what was an open question: `Beside` always creates, so this yields **two** groups, each holding one tab, not one.
...
- **A cold-start `Beside` fallback** (pre-flight column computation). OQ-5 was answered **(a)+(c)** by the repo owner on 2026-08-15: keep `Beside` unconditionally, no pre-flight, no computed column, NFR3 unrelaxed. The follow-up issue's trigger condition — **P6 shows `Beside` reuses** — did not occur (P6: always creates, § 2.4.3), so that path to a follow-up is closed; nothing here is opened.
...
| **First-ever launch lands in the user's group (`Beside` reuse), and every later session then joins it there.** | **Resolved by P6 — does not occur.** `Beside` always creates a new group rather than reusing an occupied side group (§ 2.4.3), so a first-ever launch cannot land a session tab in a pre-existing user group. Retained as a row for the record, since FR3 stated the case before it was de-risked; it is not a residual to fix. **OQ-5's (a)+(c) answer — no fallback built — stands regardless.** |
...
- **`Beside` resolution semantics are not documented in the type declarations**, though they are now empirically settled. `index.d.ts:L7776-L7777` says only *"Use `ViewColumn.Beside` to open the editor to the side of the currently active one."* Whether that reuses an existing side group was `unverified:` from the types alone; **§ 2.5 P6 settled it empirically during the Rev 5 probe session — `Beside` always creates** (§ 2.4.3).
...
- **Verified during the Rev 5 probe session** (2026-08-16, [#127 comment `5305122522`](https://github.com/glitchwerks/vscode-claude-conductor/issues/127#issuecomment-5305122522), fetched via `gh api` 2026-08-16): **P-LABEL** case (a) PASSED (the hard gate); **P6** — `Beside` always creates; **P-PLACE** — PASSED; **P-REVEAL** — PASSED on reveal; and **OQ-8** — answered yes, an editor-hosted session tab survives a window reload with its label unchanged (case (b)). None of these required a running VS Code instance from this document-editing pass — the owner ran § 2.5.1's runbook directly, which is why it existed.
- **Re-confirmed before this flip to ACCEPTED**, per the GitHub-claims bullet's own flag above (*"should be re-confirmed before the spec is flipped to ACCEPTED"*): issue #127 was re-checked via `gh issue view 127` on 2026-08-16 and remains **OPEN** with its title unchanged from the Rev 1 fetch. #68's state was not re-checked by this pass; § 4's citation of it is unaffected by this spec's acceptance.
