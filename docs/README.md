...
| [`2026-08-15-session-tab-default-grouping.md`](specs/2026-08-15-session-tab-default-grouping.md) | feature-spec | DRAFT (Rev 4) — the mechanism is a **stateless best-effort placement heuristic** (join the group holding the most `claude · `-labelled tabs, else `Beside`), unchanged since Rev 3, which removed ownership tracking rather than repairing it after two review passes broke it on the same root cause: the stable Tab API exposes no creator identity. **Rev 4 closes every design question** — OQ-1…OQ-9 answered by the repo owner 2026-08-15: D1 and D7 stay superseded, D4's redefined gate accepted (no P3 retest), **placement is always-on with no setting** (so `src/config.ts`/`package.json` leave scope), and no cold-start `Beside` fallback is built. A `project-reviewer` pass returned 0 BLOCKING; its two CONCERNs became informative probes **P-PLACE** (write-side placement) and **P-REVEAL** (`focusSession`), joining P6 alongside the one hard gate, §2.5 **P-LABEL** — now materially de-risked, since #110's P4 raw log shows settled tabs reading back as exactly `claude · <folder>`, leaving only the editor-born-vs-`moveToEditor`-born creation-path delta to close. **§2.5.1 is a step-by-step probe runbook for the owner to execute.** Blocked solely on that probe session; **not approved for implementation until P-LABEL passes** | #127 |
