...
**Status:** SUPERSEDED BY [`docs/specs/2026-08-15-session-tab-default-grouping.md`](../specs/2026-08-15-session-tab-default-grouping.md) (ACCEPTED 2026-08-16). That spec's § 2.1 records the disposition of each decision point in this plan: D2, D3, D5, D6 are carried; **D1 and D7's caching/ownership mechanism is superseded, not implemented** — Rev 3 removed it rather than repairing it — **but D1 tier b's flagged retest (the delayed-read label-predicate check) is not orphaned**: it is exactly what the spec's P-LABEL probe re-ran and passed (spec § 2.5.1, § 2.2 Rev 4 correction), so the label discriminator itself carries forward into the spec's § 2.4.2 even though D1's cache-and-validate mechanism does not. D4's follow-up `shellIntegration` retest was itself superseded by the spec's § 2.2.1 gate redefinition, confirmed by the owner (OQ-6) rather than run. **§ 8's confirmation-needed questions are likewise resolved, not left open**: item 1 (D6) and item 2 (D4) by § 2.1 above; item 3 (dedicated-group-vs-never-mixed) by the spec's FR3, which settles on the weaker "sessions congregate wherever they already are" reading; item 4 (D1 tier b) as just described; item 5 (a setting) by the spec's OQ-4 (always-on, no setting); item 6 (who runs Phase 0/probes) by OQ-9 (the repo owner, personally). Retained rather than deleted per the spec's own § 2.8, which follows the precedent at `docs/README.md:L55-L69`: its `src/`/`test/` line citations are stale (the spec re-read every one) and its content is historical evidence, not a source of current file:line facts.
...
`unverified:` VS Code starts a terminal's process lazily on first render — no supporting statement found in `node_modules/@types/vscode/index.d.ts` via a keyword search of the file; no external source was fetched to confirm or refute it during this pass. This premise is exactly what P3 exists to confirm empirically, not a settled fact this document asserts. For a terminal created with `location: {viewColumn}` **and no explicit `show()`**, verify all three:
...
|---|---|
| Editor-born terminals never start their process, so `processId` never resolves and shell integration never activates — breaking both the PID index and the fast dispatch path. | `unverified:` VS Code defers terminal process start until first render, and `unverified:` `location: {viewColumn}` without `show()` does not render — no supporting statement found in the installed `@types/vscode` declarations via a keyword search of the file; no external source was fetched to confirm or refute it during this pass. **This is the single largest threat to D4 Option A.** P3 answers it; the escape hatch is Option A + retained `show()`, then Option B. |
...
- **Repo claims** were read at commit `c65db06` (`main`, the checkout's `HEAD` at planning time), from `src/sessionManager.ts`, `src/extension.ts`, `src/quickPick.ts`, `package.json`, `test/mocks/vscode.ts`, `docs/README.md`, and `docs/sdd-workflow.md`. The citations added during the 2026-08-14 CodeRabbit-feedback pass (`docs/specs/2026-07-29-foundational-project-spec.md:L61`, `CLAUDE.md:L11`/`L18`) were read against the worktree's `HEAD` at that later date, not `c65db06`.
