---
title: Favorites Sidebar Section — implementation plan (Issue #75)
touches:
...
**Status:** ACCEPTED — derived from the locked feature spec; implementation is underway on branch `75-favorites` (open PR #77).

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a manual "Favorites" sidebar section between Active Sessions and Recent Projects, with persistent globalState storage, inline star toggling, missing-folder relocation, and a 25-entry soft cap (issue #75, verified open, fetched 2026-08-01).

**Architecture:** A standalone `FavoritesStore` service owns canonical state (lowercase index + serialized persist queue with snapshot rollback; `src/favoritesStore.ts:L57-L84,L163-L197`). A `PathExistenceCache` provides synchronous existence reads via async stat with TTL, UNC-skip, and `markMissing`/`markPresent` flips driven by `launchSession` results (`src/pathExistenceCache.ts:L80-L110`). Both `FavoritesProvider` (new) and `RecentProjectsProvider` (modified) consult the store synchronously inside `getTreeItem` to flip a star icon and `viewItem` contextValue (`src/treeView.ts:L211,L313`). A bidirectional drift-detector test parses `package.json` against an exported `VIEW_ITEM` constant set (`test/packageJsonContextKeys.test.ts:L55-L81`).

**Tech Stack:** TypeScript, VS Code Extension API (engines `^1.93.0`), vitest for tests, `vsce` for packaging (`package.json:L8-L9,L266,L274,L276`).

**Spec:** `docs/specs/2026-04-28-75-favorites-design.md` (v4 — locked after 3 inquisitor passes).
