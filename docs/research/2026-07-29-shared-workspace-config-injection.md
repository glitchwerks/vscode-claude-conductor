...
- **Relevance:** addresses requirement 1 fully (works regardless of directory nesting — a pure CLI flag Conductor already controls when constructing the dispatched command). Requirement 2 is only **partially/conditionally** addressed: the flag appends rather than replaces the default system prompt, so the appended content itself is additive at the system-prompt tier, but `--append-system-prompt` and `CLAUDE.md` load at two different tiers — system prompt versus a separate user message (see "What to avoid" below) — and no source found describes whether both are retained together for the same session. Until a probe confirms `CLAUDE.md` content survives alongside the appended system prompt, this candidate should be treated as satisfying requirement 2 conditionally, not fully. The docs do make an explicit, first-party endorsement of exactly Conductor's invocation shape: "For instructions you want at the system prompt level, use `--append-system-prompt`. This must be passed every invocation, so it's better suited to scripts and automation than interactive use." Requirement 5 (workspace-scoping) is again only indirectly reachable: the flag is per-invocation and touches no global state, which is a precondition for scoping, but the flag itself has no notion of "this VS Code workspace" — that identity would have to come from Conductor deciding, per session, which shared content to pass on the flag.
