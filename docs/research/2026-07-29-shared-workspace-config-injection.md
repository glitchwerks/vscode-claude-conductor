...
- **Relevance:** the strongest single candidate against all five requirements. Quoting directly: "The `--add-dir` flag gives Claude access to additional directories outside your main working directory... To also load memory files from additional directories, set the `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD` environment variable: `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1 claude --add-dir ../shared-config`. This loads `CLAUDE.md`, `.claude/CLAUDE.md`, `.claude/rules/*.md`, and `CLAUDE.local.md` from the additional directory." This is per-invocation (requirement 5 — Conductor already constructs the dispatched command per session), ancestry-independent (requirement 1 — works for entirely unrelated repos, not just nested ones), loads real `CLAUDE.md`/`.claude/rules/` content at the documented additional-directory tier rather than a different precedence tier (requirement 2/3), and has no hook interaction at all (requirement 4 is a non-issue).
...
- **Relevance:** addresses requirement 1 fully (works regardless of directory nesting — a pure CLI flag Conductor already controls when constructing the dispatched command), requirement 2 (appends rather than replaces the default system prompt), and requirement 5 (scoped to the single invocation Conductor builds, no global state touched). The docs make an explicit, first-party endorsement of exactly Conductor's invocation shape: "For instructions you want at the system prompt level, use `--append-system-prompt`. This must be passed every invocation, so it's better suited to scripts and automation than interactive use."
...
### 10. Manufactured-common-ancestor `CLAUDE.md` layering pattern (community blog)
...
- `user` — the manufactured-common-ancestor pattern (finding #10) is a zero-code alternative worth considering for repos whose clone location Conductor or the user controls; decide whether to recommend it as a documented convention instead of, or alongside, building an injection mechanism.
